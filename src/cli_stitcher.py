...
def stitch_clip(sentence, bin_path, index_path, output_mp4):
    ...
    # Store tuples to maintain perfect alignment across operations
    valid_word_segments = []
    ...
    for w in words:
        ...
        # Track parameters exclusively for valid words
        total_segment_cost_ms = duration_ms + GUARD_BAND_MS
        valid_word_segments.append((w, total_segment_cost_ms))
    ...
    # Accurate timeline description indexing loop
    print("\n--- YOUTUBE DESCRIPTION MAP ---")
    current_ms = 0.0
    for w, total_cost in valid_word_segments:
        seconds = int(current_ms / 1000)
        minutes = seconds // 60
        secs = seconds % 60
        print(f"{minutes:02d}:{secs:02d} - {w}")
        current_ms += total_cost
    print("--------------------------------\n")
